...
function getSiteInfo({ collection, block, rawMetadata, NOTION_CONFIG }) {
  ...
  const defaultIcon = NOTION_CONFIG?.AVATAR || BLOG.AVATAR || '/avatar.png'
  ...
  // 配置了 AVATAR（环境变量 / blog.config / Notion 配置表）时，优先生效，覆盖 Notion 数据库页面的 icon
  const hasAvatarOverride = Boolean(NOTION_CONFIG?.AVATAR || BLOG.AVATAR)
  let icon = compressImage(
    collection?.icon && !hasAvatarOverride
